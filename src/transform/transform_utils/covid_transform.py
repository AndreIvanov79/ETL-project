...
def transform_covid_batch(country: str, year_month: str, batch_date: datetime, db_manager: Any, error_manager: Any, logger: Any, validator: Any) -> int:
    ...
    total_processed = 0
    folder_path = os.path.join('..', 'extract', 'data', 'covid', country, year_month)
    ...
    for file_path in sorted(json_files):
        ...
        try:
            ...
            errors = validator.validate("covid", clean_record)
            if errors:
                country_id = db_manager.get_country_id(country)
            ...
            try:
                db_manager.insert_temp_covid_data(covid_id=covid_id, **clean_record)
            except Exception as e:
                country_id = db_manager.get_country_id(country)
    ...
    try:
        ...
        logger.error(f"Error logging final COVID transform summary: {e}")

    db_manager.execute_query(sql_templates.DROP_TEMP_COVID_TABLE)

    return total_processed


def process_covid_complete_file(country: str, file_path: str, batch_date: datetime, db_manager: Any, error_manager: Any, logger: Any, validator: Any) -> int:
    transform_id = str(uuid.uuid4())
    total_records = 0
    ...
    try:
        ...
        for record in data:
            ...
            try:
                db_manager.insert_covid_data(covid_id=covid_id, country_id=country_id, **{k: v for k, v in clean_record.items() if k != 'country_id'})
            except Exception as e:
